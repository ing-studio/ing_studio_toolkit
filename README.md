...
|---|---|---|
| [`relief_pipeline/`](relief_pipeline/README.md) | Point cloud (E57, LAS/LAZ, PLY, XYZ …) + Archicad project → relief-only Archicad project: **one terrain mesh** and **contour layers cut from it** (1 m / 3 m / 5 m by default, configurable), as plan splines and 3D lines, plus a 3D DXF of the contours. Includes an Archicad 28 add-on (Tapir palette button). | `relief_pipeline\relief.bat run --pln site.pln --cloud scan.e57` |

## Relief pipeline in short
```
relief_pipeline\relief.bat run --pln D:\site.pln --cloud D:\scan.e57                 build the relief
relief_pipeline\relief.bat run --pln D:\site.pln --cut 1 2 5 --mesh-points 30000     other sizes / lighter mesh
relief_pipeline\relief.bat run --stage archicad --force                              rebuild only the Archicad file
relief_pipeline\relief.bat addon install                                             Archicad add-on (Archicad closed)
relief_pipeline\relief.bat stages | config | test
```
...
See [relief_pipeline/README.md](relief_pipeline/README.md) for every option, the stages, and the functions of each
module. [relief_pipeline/addon/README.md](relief_pipeline/addon/README.md) covers the Archicad button.
